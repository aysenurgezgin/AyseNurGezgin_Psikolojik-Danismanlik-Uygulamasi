...
//Propgam cs te yazdığımız için OnConfiguring methoduna ihtiyacımız kalmadı.Ama base.OnConfiguring(optionsBuilder) sayesinde verdiğimz sqlite özelliğini DbContext e göndermiş oluyorduk.Şuan bu bilgiyi gönderememiş oldum.Bunun içinde bir önlem aıcam buna bir constructar yazıcam;o classtan yeni bir nesne oluşutulurken çalıştığı kodlardı.(ctor)propgram cs te varsa tip olarak bu gidecek buradan da ismi de options olabilir.base e göndermek için de: koyup base deyip(options)içine neyi göndermek istiyorsam onu yazabiliri